...
public class CucumberExtent implements EventListener {
	...
	private void handleTestCaseStarted(TestCaseStarted event) {
		...
		if ((CucumberExtentOptions.getInstance().getReportLevel() != null)
				&& (CucumberExtentOptions.getInstance().getReportLevel().equals("Feature")))
	...
	private void handleEmbed(EmbedEvent event) throws IOException {
		...
		if (extension != null) {
			StringBuilder fileName = new StringBuilder("embedded").append(embeddedIndex++)
					.append(Thread.currentThread().getId()).append(".").append(extension);
	...
	private void attachExtentHtmlReporter() {
		sparkReporter = new ExtentSparkReporter(htmlReportDir);
	...
	private void createExtentBDDScenario(ScenarioDefinition scenarioDefinition, TestCase testCase) {
		switch (scenarioDefinition.getKeyword()) {
			case "Scenario":
				extentBDDScenario = extentBDDFeature.createNode(Scenario.class, (testCase.getName() + "\n" +
	...
	private void createExtentScenario(ScenarioDefinition scenarioDefinition, TestCase testCase) {
		switch (scenarioDefinition.getKeyword()) {
			case "Scenario":
				extentTest = extent.createTest(testCase.getName() + "\n" +
						((scenarioDefinition.getDescription() != null) ? ("\n" + scenarioDefinition.getDescription())
								: ""));
				createTagList(testCase);
				break;
	...
	private void createExtentBDDSteps(Step step, PickleStepTestStep testStep, Result result) throws IOException {
		switch (step.getKeyword()) {
			case "Given ":
				stepStatus(extentBDDScenario.createNode(Given.class, testStep.getStepText()), result);
				break;
			case "When ":
				stepStatus(extentBDDScenario.createNode(When.class, testStep.getStepText()), result);
				break;
			case "Then ":
				stepStatus(extentBDDScenario.createNode(Then.class, testStep.getStepText()), result);
				break;
			case "And ":
				stepStatus(extentBDDScenario.createNode(And.class, testStep.getStepText()), result);
				break;
			default:
	...
	private void createExtentSteps(Step step, PickleStepTestStep testStep, Result result) throws IOException {
		switch (step.getKeyword()) {
			case "Given ":
				stepStatus(extentTest.createNode("Given " + testStep.getStepText()), result);
				break;
			case "When ":
				stepStatus(extentTest.createNode("When " + testStep.getStepText()), result);
				break;
			case "Then ":
				stepStatus(extentTest.createNode("Then " + testStep.getStepText()), result);
				break;
			case "And ":
				stepStatus(extentTest.createNode("And " + testStep.getStepText()), result);
				break;
			default:
				throw new CucumberException("Wrong step keyword " + step.getKeyword());
		}
